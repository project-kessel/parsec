...
//   - CEL gate off → no compliance call (AC5)
//   - CEL gate on + pass result → token issued (AC4/AC3)
//   - CEL gate on + blocked result code → 403 deny (AC4)
//   - CEL gate on + compliance service down → fail-open (AC3)
...
func TestHermeticAuthzCompliance(t *testing.T) {
	...
	t.Run("gate off does not call compliance service", func(t *testing.T) {
		...
		// No enable_compliance extension → gate off
		...
		if complianceCalls != 0 {
			t.Fatalf("expected no compliance HTTP calls when gate off, got %d", complianceCalls)
		}
	})
	...
	t.Run("gate on + pass result code → token issued", func(t *testing.T) {
		...
		authz := newAuthz(true, client)
		token := mustSignToken(t, jwksFixture, consoleClaims)
		resp, err := authz.Check(context.Background(), checkRequestWithBearerAndCompliance(token))
		if err != nil {
			t.Fatalf("Check: %v", err)
		}
		assertOKResponse(t, resp)
...
func checkRequestWithBearerAndCompliance(token string) *authv3.CheckRequest {

...
configured token types for authenticated subjects. With
`allow_anonymous_without_issue_paths`, the policy matches request paths against
RE2 regexes to allow anonymous access on configured endpoints.
...
│   │   ├── authz_check_policy.go   # Authz check policy types & StaticAuthenticatedPolicy (incl. optional-path matching)

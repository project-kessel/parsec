...
func loadRedHatIdentityLua(t *testing.T) string {
	t.Helper()
	data, err := os.ReadFile("../../configs/scripts/redhat_identity.lua")
	if err != nil {
		t.Fatalf("failed to read redhat_identity.lua: %v", err)
	}
	return string(data)
}

func newTestLuaMapper(t *testing.T, opts ...LuaMapperOption) *LuaMapper {
	t.Helper()
	script := loadRedHatIdentityLua(t)
	m, err := NewLuaMapper("redhat_identity", script, opts...)
	if err != nil {
		t.Fatalf("failed to create lua mapper: %v", err)
	}
	return m
}

func TestLuaMapper_NilInput(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_Impersonation(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_RegistryAuth(t *testing.T) {
	fixedTime := time.Date(2024, 6, 15, 10, 0, 0, 0, time.UTC)
	clk := clock.NewFixtureClock(fixedTime)
	m := newTestLuaMapper(t, WithLuaMapperClock(clk))
	...
	t.Run("with org_id", func(t *testing.T) {
		...
		expectedAuthTime := float64(fixedTime.UnixMilli())
		if internal["auth_time"] != expectedAuthTime {
			t.Errorf("expected auth_time=%v, got %v", expectedAuthTime, internal["auth_time"])
		}
	})
...
func TestLuaMapper_ServiceAccount(t *testing.T) {
	fixedTime := time.Date(2024, 6, 15, 10, 0, 0, 0, time.UTC)
	clk := clock.NewFixtureClock(fixedTime)
	m := newTestLuaMapper(t, WithLuaMapperClock(clk))
	...
	t.Run("auth_time from iat", func(t *testing.T) {
		...
		identity := mustNestedMap(t, result.Claims, "identity")
		internal := mustNestedMap(t, identity, "internal")
		expectedAuthTime := float64(1718442000000)
		if internal["auth_time"] != expectedAuthTime {
			t.Errorf("expected auth_time=%v, got %v", expectedAuthTime, internal["auth_time"])
		}
	})
...
func TestLuaMapper_ConsoleAPI(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_RHSM(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_CustomerPortal(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_UnsupportedToken(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_Precedence(t *testing.T) {
	m := newTestLuaMapper(t)
...
func TestLuaMapper_IsInternalFallback(t *testing.T) {
	m := newTestLuaMapper(t)
